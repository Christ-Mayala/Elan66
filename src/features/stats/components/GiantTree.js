...
export function GiantTree({ progress = 0 }) {
  ...
  useEffect(() => {
    const a = Animated.loop(
      Animated.sequence([
        Animated.timing(sway, { toValue: 1, duration: 1800, useNativeDriver: false }),
        Animated.timing(sway, { toValue: 0, duration: 1800, useNativeDriver: false }),
      ])
    );
    a.start();
    return () => a.stop();
  ...
  const swayDeg = useMemo(() => sway.interpolate({ inputRange: [0, 1], outputRange: ['-1.1deg', '1.1deg'] }), [sway]);

  const trunkH = grow.interpolate({ inputRange: [0, 1], outputRange: [54, 260] });
  const trunkW = grow.interpolate({ inputRange: [0, 1], outputRange: [12, 28] });
  const canopyScale = grow.interpolate({ inputRange: [0, 0.32, 1], outputRange: [0.2, 0.72, 1] });
  const canopyOpacity = grow.interpolate({ inputRange: [0, 0.2, 1], outputRange: [0, 0.9, 1] });

  const glowOpacity = pulse.interpolate({ inputRange: [0, 1], outputRange: [0.08, 0.16] });
  const glowScale = pulse.interpolate({ inputRange: [0, 1], outputRange: [0.98, 1.04] });

  const branch1Opacity = grow.interpolate({ inputRange: [0, 0.28, 0.42], outputRange: [0, 0, 1] });
  const branch2Opacity = grow.interpolate({ inputRange: [0, 0.42, 0.58], outputRange: [0, 0, 1] });
  const branch3Opacity = grow.interpolate({ inputRange: [0, 0.56, 0.74], outputRange: [0, 0, 1] });
    ...
    <View style={styles.wrap}>
      ...
      <Animated.View style={[styles.tree, { transform: [{ rotate: swayDeg }] }]}>
        ...
        >
          ...
          {LEAVES.map((leaf, idx) => {
            const o = grow.interpolate({
              inputRange: [Math.max(0, leaf.t - 0.16), leaf.t],
              outputRange: [0, 1],
            });
            const s = grow.interpolate({
          ...
          <View style={styles.leafSpark} />
          <View style={[styles.leafSpark, { left: 82, top: 74, opacity: 0.65 }]} />
          <View style={[styles.leafSpark, { left: 146, top: 98, opacity: 0.5 }]} />
        </Animated.View>
...
const styles = StyleSheet.create({
  ...
  trunk: {
    borderRadius: 999,
    backgroundColor: 'rgba(148,91,42,0.55)',
    borderWidth: 1,
    borderColor: 'rgba(148,91,42,0.40)',
    overflow: 'hidden',
  },
  trunkHighlight: {
    position: 'absolute',
  ...
  branch: {
    ...
    height: 10,
    borderRadius: 999,
    backgroundColor: 'rgba(148,91,42,0.48)',
    borderWidth: 1,
    borderColor: 'rgba(148,91,42,0.34)',
  },
  branchLeft1: { left: '50%', marginLeft: -22, bottom: 156, transform: [{ rotate: '-26deg' }] },
  branchRight1: { left: '50%', marginLeft: -52, bottom: 156, transform: [{ rotate: '26deg' }] },
  branchLeft2: { left: '50%', marginLeft: -18, bottom: 194, width: 64, transform: [{ rotate: '-18deg' }] },
  branchRight2: { left: '50%', marginLeft: -46, bottom: 194, width: 64, transform: [{ rotate: '18deg' }] },
  branchLeft3: { left: '50%', marginLeft: -14, bottom: 224, width: 52, transform: [{ rotate: '-12deg' }] },
  branchRight3: { left: '50%', marginLeft: -38, bottom: 224, width: 52, transform: [{ rotate: '12deg' }] },
  canopy: {
    position: 'absolute',
    bottom: 206,
    width: 220,
